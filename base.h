...
#define CHECK(EXP)                                                           \
  do {                                                                       \
    if (EXP) {                                                               \
      ...
      std::cout << "CHECK (" << __FILE__ << ":" << __LINE__ << "): " << #EXP \
                << std::endl;                                                \
      exit(1);                                                               \
    }                                                                        \
  } while (0)
...
template <typename T>
Optional<T> AsOptional(const T& value) {
...
class FixedArray {
 public:
  ...
  FixedArray(const FixedArray<T>& other)
      : size_(other.size_), buf_(new T[size_]) {
...
class ArrayView {
 public:
  ...
  explicit ArrayView(const FixedArray<T>* array)
      : array_(array), lo_(0), hi_(array->size()) {}
  ArrayView(const FixedArray<T>* array, int lo, int hi)
      : array_(array), lo_(lo), hi_(hi) {}
...
class Duration {
 public:
  ...
  Duration operator*(Duration other) { return Duration(nanos_ * other.nanos_); }
  Duration operator+(Duration other) { return Duration(nanos_ + other.nanos_); }
  ...
  bool operator>=(Duration other) { return nanos_ >= other.nanos_; }
  float operator/(Duration other) { return nanos_ / (double)other.nanos_; }
